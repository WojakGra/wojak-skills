...
Optional: `disable-model-invocation: true` makes the skill user-invoked only, reachable when the human types `/skill-name` and never fired autonomously. Use it for skills that start a long interactive session or cost real money. Everything else stays model-invoked.
...
Do not cross-link into another skill's files with `../other-skill/reference.md`, and do not drop a bare `/name` and hope it reads as a command. A user-invoked skill can never be called this way. When a step depends on one, instruct the human instead: "tell the user to run `/setup-x`".
...
1. Pick a bucket: `engineering/` (code work), `productivity/` (non-code workflow), `meta/` (skills about skills), `in-progress/` (public but unfinished), `deprecated/`.
2. Create `skills/<bucket>/<name>/SKILL.md` from [template.md](template.md).
3. If the bucket is promoted (`engineering`, `productivity`, `meta`), add `"./skills/<bucket>/<name>"` to the `skills` array in `.claude-plugin/plugin.json` and a row to the top-level `README.md`. Skills in `in-progress/` and `deprecated/` must appear in neither.
